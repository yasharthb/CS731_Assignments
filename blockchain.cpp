...
bool my_validate_block_helper(std::vector< std::shared_ptr<txn_t> >& transactions,
    balance_map_t& balances,
    hash_result_t& prev_hash,
    hash_result_t& reward_addr,
    hash_result_t& blk_hash,
...
{   
    ...
    uint8_t src_hash[SHA256_DIGEST_LENGTH];
    ...
    SHA256_Update(&sha256, prev_hash.data(), prev_hash.size());
    // now finalize hash.
    SHA256_Final(src_hash, &sha256);

   bool check1=true;

    for(unsigned i=0; i <SHA256_DIGEST_LENGTH; i++) {
        
        if(blk_hash.data()[i] != src_hash[i])
            {   
               // std::cout<<"\n"<<source_addr.data()[i];
                check1=false;
                break;
