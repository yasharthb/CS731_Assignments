...
bool txn_t::validate() 
{
    ...
    uint8_t src_hash[SHA256_DIGEST_LENGTH],test_hash[SHA256_DIGEST_LENGTH];

    sha256( public_key.data(), public_key.size(), src_hash);
    ...
    SHA256_Update(&sha256, change_addr.data(), change_addr.size());
    SHA256_Update(&sha256, &amount, sizeof(amount));
    SHA256_Final(test_hash, &sha256);

    //bool check1=true,check2=true;
    //std::cout<<tx_hash.data()<<"\n\n"<<test_hash<<"\n";//public_key.data()<<"\n";        //source_addr.data()<<"\n"<<src_hash;
   // std::cout<<"\nkj";
    for(unsigned i=0; i <SHA256_DIGEST_LENGTH; i++) {
        
        if(source_addr[i] != src_hash[i])
        	{   
                //std::cout<<"\n"<<source_addr.data()[i];
        	return false;
        	}
    ...
    for(unsigned i=0; i <SHA256_DIGEST_LENGTH; i++) {
        ...
        rsa_public_key_t pubKey_local(public_key.data(), public_key.size());


         if(!(pubKey_local.verify(tx_hash.data(),tx_hash.size(),tx_sign.data(),tx_sign.size())))
          {//std::cout<<"Hello";
      	 return false;
        }
